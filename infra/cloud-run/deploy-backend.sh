...
: "${PUBSUB_SUBSCRIPTION:?}"
: "${HISTORICAL_BUCKET:?}"
# Was missing here even though project review (docs/PROJECT_ZIP_REVIEW_PLAN.md)
# depends on it -- --set-env-vars below REPLACES the whole env var set on
# every deploy, so omitting a var here doesn't leave it alone, it deletes it.
...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --min-instances=1 \
  --no-cpu-throttling \
  --set-env-vars="LOCAL_MODE=false,GOOGLE_CLOUD_PROJECT=${GOOGLE_CLOUD_PROJECT},GOOGLE_CLOUD_LOCATION=${GOOGLE_CLOUD_LOCATION},GEMINI_MODEL=${GEMINI_MODEL},EMBEDDING_MODEL=${EMBEDDING_MODEL},VECTOR_SEARCH_INDEX=${VECTOR_SEARCH_INDEX},VECTOR_SEARCH_ENDPOINT=${VECTOR_SEARCH_ENDPOINT},PUBSUB_TOPIC=${PUBSUB_TOPIC},PUBSUB_SUBSCRIPTION=${PUBSUB_SUBSCRIPTION},FIRESTORE_DATABASE=${FIRESTORE_DATABASE:-},HISTORICAL_BUCKET=${HISTORICAL_BUCKET},PROJECT_FILES_BUCKET=${PROJECT_FILES_BUCKET},CORS_ORIGINS=${CORS_ORIGINS},GITHUB_CLIENT_ID=${GITHUB_CLIENT_ID},GITHUB_CLIENT_SECRET=${GITHUB_CLIENT_SECRET},GITHUB_OAUTH_REDIRECT_URI=${GITHUB_OAUTH_REDIRECT_URI}"
