...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --min-instances=1 \
  --no-cpu-throttling \
  --set-env-vars="LOCAL_MODE=false,GOOGLE_CLOUD_PROJECT=${GOOGLE_CLOUD_PROJECT},GOOGLE_CLOUD_LOCATION=${GOOGLE_CLOUD_LOCATION},GEMINI_MODEL=${GEMINI_MODEL},EMBEDDING_MODEL=${EMBEDDING_MODEL},VECTOR_SEARCH_INDEX=${VECTOR_SEARCH_INDEX},VECTOR_SEARCH_ENDPOINT=${VECTOR_SEARCH_ENDPOINT},PUBSUB_TOPIC=${PUBSUB_TOPIC},PUBSUB_SUBSCRIPTION=${PUBSUB_SUBSCRIPTION},FIRESTORE_DATABASE=${FIRESTORE_DATABASE:-},HISTORICAL_BUCKET=${HISTORICAL_BUCKET},CORS_ORIGINS=${CORS_ORIGINS}"
